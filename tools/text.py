...
def translate(js, text, tl='zh-CN'):
    ...
    header = {
        ...
        'cookie':
        '_ga=GA1.3.110668007.1547438795; _gid=GA1.3.1522575542.1548327032; 1P_JAR=2019-1-24-10; NID=156=ELGmtJHel1YG9Q3RxRI4HTgAc3l1n7Y6PAxGwvecTJDJ2ScgW2p-CXdvh88XFb9dTbYEBkoayWb-2vjJbB-Rhf6auRj-M-2QRUKdZG04lt7ybh8GgffGtepoA4oPN9OO9TeAoWDY0HJHDWCUwCpYzlaQK-gKCh5aVC4HVMeoppI',
        # 'cookie': '',
        'user-agent':
        'Mozilla/5.0 (Windows NT 10.0; WOW64)  AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.108 Safari/537.36',
        'x-client-data':
        'CKi1yQEIhrbJAQijtskBCMG2yQEIqZ3KAQioo8oBCL+nygEI7KfKAQjiqMoBGPmlygE='
    }
    ...
    try:
        ...
        if res is None:
            if result[7] is not None:
                ...
                try:
                    correct_text = result[7][0].replace('<b><i>', ' ').replace(
                        '</i></b>', '')
                    if debug:
                        print(correct_text)
                    correct_url = build_url(correct_text,
                                            js.get_tk(correct_text), tl)
...
def get_translate_words(word, tl='zh-CN'):
    results = get_translate(word, tl)
    translate_result = ""
    # print(results)

    if "." in word or "?" in word:
        for result in results:
            translate_result += str(result[0])
    else:
        translate_result = str(results[0][0])

    # 删除分词造成的结尾None
    if translate_result.endswith('None'):
        translate_result = translate_result[:-4]
...
if __name__ == '__main__':
    ...
    with open("./DataSource/my.txt", 'r', encoding='utf8') as file:
        ...
        for word in words:
            ...
            outfile.writelines(word.replace(content, ' ' + results + ' '))
